 ...
 * Run: npx ts-node scripts/audit-cj-shopify-products.ts
 */
...
async function auditProducts() {
  ...
  const draftListings = products.filter(p => 
    p.listings.some(l => l.status === 'DRAFT') && !p.listings.some(l => l.status === 'ACTIVE')
  );
  const failedListings = products.filter(p => 
    p.listings.some(l => l.status === 'FAILED')
  );
  const noListings = products.filter(p => p.listings.length === 0);
  ...
  for (const product of products) {
    const activeCount = product.listings.filter(l => l.status === 'ACTIVE').length;
    const draftCount = product.listings.filter(l => l.status === 'DRAFT').length;
    const failedCount = product.listings.filter(l => l.status === 'FAILED').length;
