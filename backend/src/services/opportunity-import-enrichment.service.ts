...
export async function enrichProductAfterOpportunityImport(
  ...
  if (!api.isConfigured()) {
    const productData = mergeProductDataRoot(product.productData, {
      opportunityImport: {
        affiliateEnrichment: 'skipped',
        affiliateEnrichmentReason: 'affiliate_api_not_configured',
        affiliateEnrichmentAt: new Date().toISOString(),
      },
    });
    await prisma.product.update({
    ...
    return { ok: false, reason: 'affiliate_api_not_configured' };
  }
  ...
  try {
    ...
    const productData = mergeProductDataRoot(product.productData, {
      opportunityImport: {
        affiliateEnrichment: 'failed',
        affiliateEnrichmentReason: err?.message || 'affiliate_api_error',
        affiliateEnrichmentAt: new Date().toISOString(),
      },
    });
    await prisma.product.update({
    ...
    return { ok: false, reason: 'affiliate_api_error', skuResolved: !!skuId };
  }
  ...
  const productData = mergeProductDataRoot(product.productData, {
    opportunityImport: {
      affiliateEnrichment: skuId && shippingNum !== null ? 'ok' : skuId ? 'partial' : 'partial',
      affiliateEnrichmentAt: new Date().toISOString(),
      affiliateEnrichmentReason: null,
    ...
    preventivePublish: {
      selectedSupplier: {
        productId: aeId,
        skuId: skuId || null,
        source: 'opportunity_import_affiliate',
      },
    },
  });
