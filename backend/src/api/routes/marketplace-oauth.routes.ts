...
function parseState(state: string) {
  try {
    ...
    const [userIdStr, mk, ts, nonce, redirB64, sig] = parts;
    const payload = [userIdStr, mk, ts, nonce, redirB64].join('|');
    ...
    return { ok: true, userId, marketplace: mk, redirectUri };
  } catch {
    return { ok: false };
  }
}
...
router.get('/oauth/callback/:marketplace', async (req: Request, res: Response) => {
  try {
    ...
    const { userId, redirectUri } = parsed as any;
    ...
    if (marketplace === 'ebay') {
      ...
      await marketplaceService.saveCredentials(userId, 'ebay', newCreds);
      ...
      await marketplaceService.saveCredentials(userId, 'mercadolibre', newCreds);
    } else {
