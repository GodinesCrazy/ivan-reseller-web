#!/usr/bin/env tsx
/**
 * Platform commission verifier: POST /api/internal/test-platform-commission
 * Exit 0 only if success === true (commission applied and both payouts succeeded).
 */
...
async function main(): Promise<number> {
  ...
  try {
    ...
    if (data.success === true) {
      console.log('Platform commission PASSED');
      return 0;
    }

    console.error('Platform commission FAILED');
    return 1;
  } catch (err: unknown) {
