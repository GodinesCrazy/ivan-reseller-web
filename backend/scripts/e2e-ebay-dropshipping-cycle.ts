...
async function main() {
  ...
  try {
    ...
    if (err.statusCode === 409 && err.details?.existingProductId) {
      const existingId = err.details.existingProductId;
      console.log(`   ⚠️  Product already exists: ID=${existingId}. Using existing product...`);
      product = await prisma.product.findUnique({ where: { id: existingId } });
    } else {
      throw err;
    }
  }
