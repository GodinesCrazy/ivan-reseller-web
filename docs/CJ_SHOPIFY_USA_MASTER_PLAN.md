...
- Current warnings: webhook subscriptions are empty and checkout reality remains third-party for a Chile-based operator. See `CJ_SHOPIFY_USA_READINESS.md`.
...
1. **Webhook subscriptions** — `ORDERS_CREATE` and `APP_UNINSTALLED` are not currently present in `auth/test`; run `POST /api/cj-shopify-usa/webhooks/register` and revalidate.
...
1. Run `POST /api/cj-shopify-usa/webhooks/register`.
2. Re-run `POST /api/cj-shopify-usa/auth/test` and confirm webhook topics appear.
3. Continue using Discover to search a product, evaluate it, create a draft, and publish through Store Products.
4. Verify the published product appears in Shopify admin / storefront.
5. Place a controlled test order and verify webhook ingestion plus tracking sync.
