...
const API_DEFINITIONS: Record<string, APIDefinition> = {
  ...
  amazon: {
    ...
    fields: [
      ...
      { key: 'AMAZON_MARKETPLACE_ID', label: 'Marketplace ID', required: true, type: 'text', placeholder: 'ATVPDKIKX0DER (US)', helpText: 'Ej: ATVPDKIKX0DER (US), A2EUQ1WTGCTBG2 (CA), A1AM78C64UM0Y8 (MX)' },
      { key: 'AMAZON_SELLER_ID', label: 'Seller ID', required: false, type: 'text', placeholder: 'A2XXXXXXXXXX' },
      { key: 'AMAZON_ACCESS_KEY_ID', label: 'AWS Access Key ID', required: false, type: 'password', placeholder: 'AKIA...', helpText: 'Opcional si usas AMAZON_ACCESS_KEY en .env' },
      { key: 'AMAZON_SECRET_ACCESS_KEY', label: 'AWS Secret Access Key', required: false, type: 'password', placeholder: '...', helpText: 'Opcional si usas AMAZON_SECRET_KEY en .env' },
    ],
  },
  mercadolibre: {
    name: 'mercadolibre',
  ...
  const handleSave = async (apiName: string) => {
    ...
    try {
      ...
      const fieldMapping: Record<string, string> = {
        ...
        'AMAZON_ACCESS_KEY_ID': 'awsAccessKeyId',
        'AMAZON_SECRET_ACCESS_KEY': 'awsSecretAccessKey',
  ...
  const handleOAuth = async (apiName: string, environment: string) => {
    ...
    try {
      ...
      if (existing?.apiName === apiName && existing?.win && !existing.win.closed) {
        try { existing.win.focus(); } catch (_) {}
        delete oauthInProgressRef.current[apiName];
        setOauthing(null);
        return;
      }
      openOAuthWindowRef.current = null;
      // ✅ Marcar "abriendo" de inmediato para que un segundo clic no abra otra ventana
      const openTarget = 'oauth_' + apiName;
      openOAuthWindowRef.current = { apiName, win: null as unknown as Window };
      let oauthWindow: Window | null = null;
      try {
        oauthWindow = window.open(authUrl, openTarget, 'noopener,noreferrer,width=500,height=700');
      ...
      if (oauthWindow === null || oauthWindow === undefined) {
        ...
        try { toast('Abriendo OAuth en esta pestaña...', { icon: 'ℹ️' }); } catch (_) {}
        window.location.href = authUrl;
        setOauthing(null);
        return;
      }
      
      log.info('[APISettings] OAuth window opened successfully', { apiName });
