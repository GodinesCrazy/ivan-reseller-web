  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle={operationsTruth?.generatedAt
          ? `Listings, blockers, pruebas comerciales y decisiones de agentes. Evidencia: ${new Date(operationsTruth.generatedAt).toLocaleString()}`
        ...
        badge={
          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${
            ...
            {environment === 'production' ? 'Produccion' : 'Sandbox'}
          </span>
        }
      />
        ...
        <>
            ...
            <div className="ir-panel p-4">
              <div className="mb-4">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Bloqueos y siguientes pasos tomados directamente del contrato canonico.
                </p>
              </div>
                  ...
                  {actionItems.map((item) => (
                    <div key={item.productId} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 p-3">
                      {(() => {
                        ...
                        return (
                          <div className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium mb-2 ${lifecycleToneClasses(lifecycle.tone)}`}>
                            {lifecycle.label}
                          </div>
                        );
                      })()}
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {item.blockerCode ? (
                          <span className="inline-flex items-center gap-1 rounded-full bg-red-100 dark:bg-red-900/30 px-2 py-0.5 text-[11px] font-medium text-red-700 dark:text-red-300 shrink-0">
                            <ShieldAlert className="h-3 w-3" />
                            {item.blockerCode}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 dark:bg-blue-900/30 px-2 py-0.5 text-[11px] font-medium text-blue-700 dark:text-blue-300 shrink-0">
                      ...
                      {item.nextAction && (
                        <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                          Siguiente accion: {item.nextAction}
                        </p>
                      )}
                      {item.lastMarketplaceSyncAt && (
                        <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                          Ultima evidencia: {new Date(item.lastMarketplaceSyncAt).toLocaleString()}
                        </p>
                      )}
                    </div>
                  ))}
          ...
          <div className="ir-panel p-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                Listing → Orden → Fulfillment: producto, listing, orden ingerida y evidencia de fulfillment.
              </p>
            </div>
            {watchRows.length === 0 ? (
                ...
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-800">
                      ...
                      <th className="ir-table-header">Ciclo de vida</th>
                      <th className="ir-table-header">Listing</th>
                      <th className="ir-table-header">Sincr. Orden</th>
                      <th className="ir-table-header">Fulfillment</th>
                      <th className="ir-table-header">Siguiente accion</th>
                    </tr>
                  </thead>
                  <tbody>
                    {watchRows.map((row) => {
                      ...
                      return (
                        <tr key={row.productId} className="ir-table-row">
                          <td className="ir-table-cell py-2.5 pr-3">
                            <div className="font-medium text-slate-900 dark:text-slate-100 text-sm">{row.productTitle}</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500">#{row.productId}</div>
                          </td>
                          <td className="ir-table-cell py-2.5 pr-3">
                          ...
                          <td className="ir-table-cell py-2.5 pr-3">
                            <div className="text-sm text-slate-700 dark:text-slate-300">{row.marketplace || '—'}</div>
                            <div className="font-mono text-xs text-slate-400 dark:text-slate-500">{row.listingId || '—'}</div>
                          </td>
                          <td className="ir-table-cell py-2.5 pr-3">
                            {row.orderIngested ? (
                              <span className="text-emerald-600 dark:text-emerald-400 text-xs font-medium">registrada</span>
                            ) : (
                          ...
                          <td className="ir-table-cell py-2.5 pr-3">
                            <div className="text-xs text-slate-600 dark:text-slate-400">
                              Compra: {row.supplierPurchaseProved ? <span className="text-emerald-600 dark:text-emerald-400">confirmada</span> : <span className="text-amber-600 dark:text-amber-400">pendiente</span>}
                            </div>
                            <div className="text-xs text-slate-600 dark:text-slate-400">
                              Tracking: {row.trackingAttached ? <span className="text-emerald-600 dark:text-emerald-400">adjunto</span> : <span className="text-amber-600 dark:text-amber-400">falta</span>}
                            </div>
                          </td>
                          <td className="ir-table-cell py-2.5 text-xs text-amber-700 dark:text-amber-300">
      ...
      <div className="ir-panel p-5">
        <div className="mb-4">
          <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            Controles de Automatizacion
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Controles reales de ejecucion. Disparan flujos tecnicos, pero no equivalen por si solos a exito comercial o listings activos.
          </p>
        </div>
