...
function nodeClass(state: ProductLifecycleStepState): string {
  if (state === 'done') return 'border-emerald-500 bg-emerald-500 text-white shadow-sm shadow-emerald-900/30';
  if (state === 'active') return 'border-cyan-400 bg-cyan-400 text-slate-950 shadow-sm shadow-cyan-900/40 animate-pulse';
  if (state === 'blocked') return 'border-rose-400 bg-rose-500 text-white shadow-sm shadow-rose-900/30';
        ...
        {steps.map((step, index) => (
          <div key={step.key} className="flex items-center">
            <div className="flex flex-col items-center" title={step.title ?? `${step.label}: ${step.state}`}>
              <span className={`flex ${nodeSize} cursor-default items-center justify-center rounded-full border-2 font-bold transition-all duration-300 motion-safe:hover:scale-125 ${nodeClass(step.state)}`}>
                {step.state === 'done' ? (
              ...
              <span className={`mt-1.5 truncate font-semibold leading-none text-slate-500 ${labelClass}`}>{step.label}</span>
            </div>
            {index < steps.length - 1 && (
              <span className={`mb-5 h-[3px] ${connectorWidth} overflow-hidden rounded-full bg-slate-800`}>
                <span
