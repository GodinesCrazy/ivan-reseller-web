...
export default function CjEbayAlertsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<{ ok: boolean; alerts: Alert[] }>(`/api/cj-ebay/alerts${qs}`);
  ...
  const acknowledge = async (id: number) => {
    ...
    setActionMsg(null);
    try {
      await api.post(`/api/cj-ebay/alerts/${id}/acknowledge`);
  ...
  const resolve = async (id: number) => {
    ...
    setActionMsg(null);
    try {
      await api.post(`/api/cj-ebay/alerts/${id}/resolve`);
      ...
      {!loading && !error && alerts.length === 0 && (
        <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-600 bg-slate-50/80 dark:bg-slate-900/30 px-5 py-10 text-center">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
            Las alertas se generan automáticamente cuando ocurren eventos como pago bloqueado, refund activo o tracking ausente.
          </p>
        </div>
      )}
                ...
                <div className="flex-1 min-w-0 space-y-0.5">
                  ...
                  {payload && (
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                      {Boolean(payload['ebayOrderId']) && (
                        <span className="text-[11px] text-slate-500 dark:text-slate-400">
                          eBay order: <span className="font-mono">{String(payload['ebayOrderId'])}</span>
                        </span>
                      )}
